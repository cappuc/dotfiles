...
2. [Generate a new public and private SSH key](https://docs.github.com/en/github/authenticating-to-github/generating-a-new-ssh-key-and-adding-it-to-the-ssh-agent) by running:
   ...
   ```zsh
   curl https://raw.githubusercontent.com/cappuc/dotfiles/HEAD/ssh.sh | sh -s "<your-email-address>"
   ```

3. Clone this repo to `~/.dotfiles` with:

    ```zsh
    git clone --recursive git@github.com:cappuc/dotfiles.git ~/.dotfiles
    ```
...
Check out the [`aliases.zsh`](./aliases.zsh) file and add your own aliases. If you need to tweak your `$PATH` check out the [`path.zsh`](./path.zsh) file. These files get loaded by the `for f in $DOTFILES/*.zsh` loop in [`.zshrc`](./.zshrc), which you can adjust to your liking. The prompt is [Starship](https://starship.rs).
